...
processbatch(char *filename)
{
    ...
    f = fopen(filename, "r");
    if (!f) {
	fprintf(stderr, "unable to open batchfile %s (%s) -- abort\n",
	...
	if (0 != strncmp(l, tomatch, strlen(tomatch))) {
	    fprintf(stderr, "expected `#! rnews', got `%-.40s[...]'", l);
	    goto pb_bail;
	}
	...
	if (fclose(artfile)) {
	    fprintf(stderr, "cannot close file: %s", strerror(errno));
	    return 0;
	}
	processfile(artname);
    }
 ...
 * otherwise name of compressed file without suffix
 */
static char *
makecompressedfile(FILE * infile)
{
    FILE *outfile;
    char *buf;
    char *c = TEMPLATE;		/* will be munged by mkstemp */
    char *rc = c;
    size_t i;
    int cfd;
    int ofd;
    ...
    sprintf(s, "%s.gz", c);	/* FIXME: this is insecure */
    ...
    if (!outfile) {
	fprintf(stderr, "unable to fdopen filedescriptor %d: %s\n",
		ofd, strerror(errno));
	close(cfd);
	close(ofd);
	return NULL;
    }
    buf = critmalloc((size_t) BLOCKSIZE + 1, "Allocating memory");
    do {
	...
	if (fwrite(buf, sizeof(char), i, outfile) < i) {
...
uncompressfile(char *filename)
{
	...
	fprintf(stderr, "Decompressing failed: waitpid returned error: %s\n",
		strerror(errno));
    }
 ...
 */
static int
fprocessfile(FILE * f, char *filename)
{
    char *l;
    char *tempfilename = 0;
    int i;
