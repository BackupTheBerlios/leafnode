...
void
ln_log(int sev, const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
    vln_log_core(1, stderr, sev, format, ap);
    va_end(ap);
}

void
ln_log_so(int sev, const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
    vln_log_core(1, stdout, sev, format, ap);
    va_end(ap);
}

void
ln_log_prt(int sev, const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
    vln_log_core(0, stderr, sev, format, ap);
    va_end(ap);
}

void
ln_log_sys(int sev, const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
    vln_log_core(1, 0, sev, format, ap);
    va_end(ap);
}
